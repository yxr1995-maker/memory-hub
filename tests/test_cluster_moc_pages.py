...
def test_classify_cluster_target_branches(tmp_path: Path):
    target = tmp_path / "moc" / "cluster-abc.md"
    assert classify_cluster_target(target, b"x", None) == "write"
    target.parent.mkdir(parents=True)
    target.write_bytes(b"content")
    assert classify_cluster_target(target, b"content", None) == "idempotent"
    assert classify_cluster_target(target, b"other", None) == "manual"
...
def test_aggregate_rerun_never_overwrites(tmp_path: Path):
    ...
    assert second.data["skipped"] == [page.relative_to(wiki).as_posix()]
