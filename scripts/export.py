...
将 ~/llm-wiki 知识库中的页面按项目、类型结构化导出为 JSONL、JSON 或合并 Markdown 归档。

用法:
  memory-hub.sh export [--project NAME] [--type TYPE] [--format jsonl|json|markdown] [--output FILE]
"""
...
def export_pages(
    ...
    if fmt == "jsonl":
        ...
        for p in pages:
            ...
            out_sections.append(p.get("body", "").strip() + "\n\n---\n")
        return "\n".join(out_sections)
    else:
...
def main() -> int:
    ...
    pages = collect_pages(wiki_path, project_filter=args.project, type_filter=args.type)
